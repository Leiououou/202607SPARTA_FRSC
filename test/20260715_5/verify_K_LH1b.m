% verify_K_LH1b.m
% 验证带体相的 LH1: O(g) + O(b) --> O2(g) 的 K_DSMC vs K_theory
% 核心验证点: 体相反应物 O(b) 的 Gamma_b = 1 (数密度视为常数, 不修正概率)
%
% 与 20260715_2 (O(g)+O(s)-->O2(g)) 的关键区别:
%   s 相反应物换成了 O(b), 概率只含【一个】表面密度因子 (空位 n(S), 来自 coeff[3]=1):
%   P(Vn) = min(1, 2 * K * n(S) / Vn)          <-- O(b) 贡献 Gamma_b = 1
% 与 20260715_4 (LH3 带体相) 公式完全相同, 区别仅在产物 O2(g) 返回气相
%
% === 两种 K 反算方法 ===
%
...
%
% 方法2 — 反应速率定义直接反算 (n_Ob 按约定取常数 1, 已吸收进 A):
%   dn_o2/dt = K * n_og * n_s * n_Ob = K * n_og * n_s
...
%% ====== 反应参数 (来自 schu_gs_1.surf: LH1 A 5.2e-20 0.5 5000 1) ======
A   = 5.2e-20;
b   = 0.5;
Ea  = 5000;              % K
...
% Surface reaction tallies: reaction O(g) + O(b) --> O2(g)
N_react = [280; 738; 1514; 2703; ...
           4522; 6952; 9947; 13719; ...
           17872; 22748; 28143; 34192; ...
           40758; 47685];

% Boundary collides (4 个边界面总数, /4 得 xhi 反应面碰撞数)
% 与 20260715_2/3/4 逐点相同 (相同 seed 和气相工况, tally_only 不改状态)
...
% ---- 方法2: dn_o2/dt = K * n_og * n_s  直接反算 (n_Ob=1) ----
dno2_dt = N_react * fnum ./ (se * t_total);     % N_react 为模拟粒子数, x fnum 换算真实反应数
K_DSMC_rate = dno2_dt ./ (n_og * n_S * n_Ob);   % K = (dn_o2/dt) / (n_og * n_s * n_Ob)
...
%% ====== 输出表格 ======
fprintf('========== LH1(带体相): O(g)+O(b)-->O2(g)  K_DSMC 验证 ==========\n');
fprintf('反应参数: A=%.1e, b=%.1f, Ea=%d K\n', A, b, Ea);
fprintf('n_og = %.4e m^-3,  n(S) = %.4e m^-2  (%.0f%% cover),  n_Ob = %g (常数约定)\n', ...
    n_og, n_S, cov_frac*100, n_Ob);
...
% (4) dno2/dt vs T
subplot(2,3,4);
plot(T, dno2_dt, 'co-', 'LineWidth', 1.5, 'MarkerSize', 6);
xlabel('T (K)'); ylabel('dn_{o2}/dt (m^{-2}s^{-1})');
title('反应速率 dn_{o2}/dt');
grid on;
...
sgtitle('LH1(带体相): O(g)+O(b)\rightarrowO_2(g)  --  A=5.2e-20, init\_cover=0.5, tally\_only, \Gamma_b=1');

%% ====== 保存数据 ======
save('verify_K_LH1b_data.mat', 'T', 'P_bar', 'N_react', 'N_coll', ...
     'K_DSMC_eq52', 'K_DSMC_rate', 'K_theory', ...
     'ratio_eq52', 'ratio_rate', 'dno2_dt', ...
     'a_cap', 'C_cap', 'P_capped', 'ratio_corr', ...
     'n_S', 'n_Ob', 'n_og', 'se', 't_total', 'N_site');
fprintf('\n数据已保存至 verify_K_LH1b_data.mat\n');

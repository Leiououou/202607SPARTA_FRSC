% verify_K_LH1.m
% 验证 LH1: O(g) + O(s) --> O2(g) 的 K_DSMC vs K_theory
...
%
% 方法2 — 反应速率定义直接反算:
%   dn_os/dt = K * n_og * n_os * n_s
%   K = (dn_os/dt) / (n_og * n_os * n_s)
...
%% ====== 反应参数 (来自 schu_gs_1.surf: LH1 A 5.2e-39 0.5 5000 1) ======
A   = 5.2e-39;
b   = 0.5;
Ea  = 5000;              % K

%% ====== 表面参数 (来自 in.LH1: max_cover 6.022e18, init_cover O 0.5) ======
...
%% ====== 从 log.sparta 提取 (2026-07-16 重跑, A=5.2e-39, 5000 步) ======
% Surface reaction tallies: reaction all
N_react = [93; 221; 450; 820; ...
           1331; 2119; 2923; 4077; ...
           5379; 6787; 8534; 10282; ...
           12224; 14293];
...
% ---- 方法2: dn_os/dt = K * n_og * n_os * n_s  直接反算 ----
dnos_dt = N_react * fnum ./ (se * t_total);      % N_react 为模拟粒子数, x fnum 换算真实反应数
K_DSMC_rate = dnos_dt ./ (n_og * n_S * n_O_s);   % K = (dnos/dt) / (n_og * n_os * n_s)
...
%% ====== 输出表格 ======
fprintf('========== LH1: O(g)+O(s)-->O2(g)  K_DSMC 验证 ==========\n');
...
% (4) dnos/dt vs T
subplot(2,3,4);
plot(T, dnos_dt, 'co-', 'LineWidth', 1.5, 'MarkerSize', 6);
xlabel('T (K)'); ylabel('dn_{os}/dt (m^{-2}s^{-1})');
title('反应速率 dn_{os}/dt');
grid on;
...
sgtitle('LH1: O(g)+O(s)\rightarrowO_2(g)  --  A=5.2e-39, init\_cover=0.5, tally\_only');

%% ====== 保存数据 ======
save('verify_K_LH1_data.mat', 'T', 'P_bar', 'N_react', 'N_coll', ...
     'K_DSMC_eq52', 'K_DSMC_rate', 'K_theory', ...
     'ratio_eq52', 'ratio_rate', 'dnos_dt', ...
     'a_cap', 'C_cap', 'P_capped', 'ratio_corr', ...
     'n_S', 'n_O_s', 'n_og', 'se', 't_total', 'N_site');
fprintf('\n数据已保存至 verify_K_LH1_data.mat\n');

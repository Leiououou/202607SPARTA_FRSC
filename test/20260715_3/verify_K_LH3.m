% verify_K_LH3.m
% 验证 LH3: O(g) + O(s) --> O2(s) 的 K_DSMC vs K_theory
% init_cover O 0.5: n(S) = n(O_s) = 0.5 * max_cover
%
% LH3 与 LH1 概率公式完全相同 (surf_react_adsorb.cpp):
%   P(Vn) = min(1, 2 * K * n(S) * n(O_s) / Vn)
% 区别仅在产物: LH1 生成 O2(g), LH3 生成 O2(s) (留在表面)
%
% === 两种 K 反算方法 ===
%
% 方法1 — Molchanova Eq.(52) 碰撞概率反算:
%   P(Vn) = min(1, 2 * K * n(S) * n(O_s) / Vn)
%   系综平均: K = P_bar / (2 * n(S) * n(O_s) * <1/Vn>)
%            = (N_react ./ N_coll) ./ (2 * n_S * n_O_s .* Vn_1)
%   <1/Vn> = sqrt(pi*m/(2*k*T))
%
% 方法2 — 反应速率定义直接反算:
%   dn_o2s/dt = K * n_og * n_os * n_s
%   K = (dn_o2s/dt) / (n_og * n_os * n_s)
%     = (N_react * fnum / se / t_total) / (n_og * n_S * n_O_s)
...
%% ====== 反应参数 (来自 schu_gs_1.surf: LH3 A 5.2e-39 0.5 5000 1) ======
A   = 5.2e-39;
b   = 0.5;
Ea  = 5000;              % K
...
cov_frac  = 0.5;                % init_cover fraction
n_S       = cov_frac * N_site;  % 空位 S 数密度 (m^-2)
n_O_s     = cov_frac * N_site;  % O(s) 数密度 (m^-2)

%% ====== 模拟温度 (K) ======
T = [700, 800, 900, 1000, 1100, 1200, 1300, 1400, ...
     1500, 1600, 1700, 1800, 1900, 2000]';

%% ====== 从 log.sparta 提取 (2026-07-16, A=5.2e-39, 5000 步, tally_only) ======
% Surface reaction tallies: reaction O(g) + O(s) --> O2(s)
N_react = [78; 220; 461; 886; ...
           1399; 2211; 3141; 4238; ...
           5513; 7052; 8681; 10473; ...
           12501; 14525];
...
% ---- 方法1: Molchanova Eq.(52) 碰撞概率反算 ----
K_DSMC_eq52 = (N_react ./ N_coll) ./ (2 * n_S * n_O_s .* Vn_1);

% ---- 方法2: dn_o2s/dt = K * n_og * n_os * n_s  直接反算 ----
dno2s_dt = N_react * fnum ./ (se * t_total);      % N_react 为模拟粒子数, x fnum 换算真实反应数
K_DSMC_rate = dno2s_dt ./ (n_og * n_S * n_O_s);   % K = (dn_o2s/dt) / (n_og * n_os * n_s)
...
%% ====== 输出表格 ======
fprintf('========== LH3: O(g)+O(s)-->O2(s)  K_DSMC 验证 ==========\n');
fprintf('反应参数: A=%.1e, b=%.1f, Ea=%d K\n', A, b, Ea);
fprintf('n_og = %.4e m^-3,  n(S) = n(O_s) = %.4e m^-2  (%.0f%% cover)\n', ...
    n_og, n_S, cov_frac*100);
...
fprintf('\n===== min(1,P) 截断修正 =====\n');
fprintf('高温时 a=2K·nS·nOs 接近热速度, 慢分子概率被 min(1,·) 截断为 1\n');
fprintf('  T(K)    a(m/s)    a/s      C(T)    P_bar实测   P_capped解析   ratio修正后\n');
fprintf('  ----    ------    ---      ----    --------    ----------    ----------\n');
for i = 1:length(T)
    fprintf('%5d    %6.1f    %.3f    %.4f    %.4e    %.4e    %8.3f\n', ...
        T(i), a_cap(i), a_cap(i)/s_th(i), C_cap(i), P_bar(i), P_capped(i), ratio_corr(i));
end
fprintf('\n若 P_bar实测 ≈ P_capped解析 且 ratio修正后 ≈ 1, 则偏差完全由截断解释, 代码实现正确\n');
...
% (2) N_react vs T
subplot(2,3,2);
plot(T, N_react/1e6, 'bo-', 'LineWidth', 1.5, 'MarkerSize', 6);
xlabel('T (K)'); ylabel('N_{react} (10^6)');
title('反应计数');
grid on;
...
% (4) dno2s/dt vs T
subplot(2,3,4);
plot(T, dno2s_dt, 'co-', 'LineWidth', 1.5, 'MarkerSize', 6);
xlabel('T (K)'); ylabel('dn_{o2s}/dt (m^{-2}s^{-1})');
title('反应速率 dn_{o2s}/dt');
grid on;
...
sgtitle('LH3: O(g)+O(s)\rightarrowO_2(s)  --  A=5.2e-39, init\_cover=0.5, tally\_only');

%% ====== 保存数据 ======
save('verify_K_LH3_data.mat', 'T', 'P_bar', 'N_react', 'N_coll', ...
     'K_DSMC_eq52', 'K_DSMC_rate', 'K_theory', ...
     'ratio_eq52', 'ratio_rate', 'dno2s_dt', ...
     'a_cap', 'C_cap', 'P_capped', 'ratio_corr', ...
     'n_S', 'n_O_s', 'n_og', 'se', 't_total', 'N_site');
fprintf('\n数据已保存至 verify_K_LH3_data.mat\n');

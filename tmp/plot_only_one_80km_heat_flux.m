%% Orion 80 km only_one heat-flux post-processing
% Compares the new single-site only_one model against the conventional
% constant-gamma model, the original gamma_CO=0 result, NC, and paper data.
% The surf file with the largest numeric timestep is selected automatically.
% The latest snapshot is also compared with the preceding available snapshot
% so an updated long run can be checked for heat-flux convergence directly.
...
%% User settings
onlyOneDir = 'D:\\博一\\catalytic\\z_0804';
referenceDir = 'D:\\博一\\catalytic\\zupparid';

caseInfo = struct( ...
    'name', {'only-one', 'conventional', 'FC-original', 'gamma0', 'NC'}, ...
    'root', {onlyOneDir, referenceDir, referenceDir, referenceDir, referenceDir}, ...
    'folder', {'80_gCO', '80_gCO', '80', '80_gamma_0', '80_nogamma'}, ...
    'label', {'only\_one yes, \gamma_{CO}=1', ...
              'Conventional \gamma model, \gamma_{CO}=1', ...
              'Original FC, \gamma_{CO}=0', ...
              '\gamma=0', 'NC: no surf\_react'});
...
paperFCFile = fullfile(onlyOneDir, '80kmFC热流.csv');
paperNCFile = fullfile(onlyOneDir, '80kmNC热流.csv');
...
for iCase = 1:nCases
    caseDir = fullfile(caseInfo(iCase).root, caseInfo(iCase).folder);
...
sCommon = result{5}.s;
qOnlyOne = interpolateToCommonGrid(result{1}, sCommon);
qConventional = interpolateToCommonGrid(result{2}, sCommon);
qOriginalFC = interpolateToCommonGrid(result{3}, sCommon);
qGamma0 = interpolateToCommonGrid(result{4}, sCommon);
qNC = result{5}.heatFlux;
qReactionOriginal = qOriginalFC - qNC;
qReactionConventional = qConventional - qNC;
qReactionOnlyOne = qOnlyOne - qNC;
qOnlyOneDifference = qOnlyOne - qConventional;
...
[qOriginalFCMax, iOriginalFCMax] = max(qOriginalFC);
[qConventionalMax, iConventionalMax] = max(qConventional);
[qOnlyOneMax, iOnlyOneMax] = max(qOnlyOne);
[qNCMax, iNCMax] = max(qNC);
[qReactionMax, iReactionMax] = max(qReactionOnlyOne);
isPlottedRange = sCommon <= plotArcLengthMax;
plottedIndices = find(isPlottedRange);
[qOnlyOneDifferenceMin, iDifferenceMinLocal] = ...
    min(qOnlyOneDifference(isPlottedRange));
[qOnlyOneDifferenceMax, iDifferenceMaxLocal] = ...
    max(qOnlyOneDifference(isPlottedRange));
iOnlyOneDifferenceMin = plottedIndices(iDifferenceMinLocal);
iOnlyOneDifferenceMax = plottedIndices(iDifferenceMaxLocal);
fprintf('\nPeak values on the TPS:\n');
fprintf('  Original FC (gamma_CO=0): %.6g W/m^2 at s = %.6g m\n', ...
    qOriginalFCMax, sCommon(iOriginalFCMax));
fprintf('  Conventional gamma_CO=1: %.6g W/m^2 at s = %.6g m\n', ...
    qConventionalMax, sCommon(iConventionalMax));
fprintf('  only_one gamma_CO=1: %.6g W/m^2 at s = %.6g m\n', ...
    qOnlyOneMax, sCommon(iOnlyOneMax));
fprintf('  NC: %.6g W/m^2 at s = %.6g m\n', qNCMax, sCommon(iNCMax));
fprintf('  q_r for only_one: %.6g W/m^2 at s = %.6g m\n', ...
    qReactionMax, sCommon(iReactionMax));
fprintf('  only_one - conventional over 0 <= s <= %.3g m:\n', ...
    plotArcLengthMax);
fprintf('    minimum = %.6g W/m^2 at s = %.6g m\n', ...
    qOnlyOneDifferenceMin, sCommon(iOnlyOneDifferenceMin));
fprintf('    maximum = %.6g W/m^2 at s = %.6g m\n', ...
    qOnlyOneDifferenceMax, sCommon(iOnlyOneDifferenceMax));

%% Figure 1: paper-style physical comparison
fig1 = figure('Color', 'w', 'Position', [80, 80, 1280, 760]);
ax1 = axes(fig1);
hold(ax1, 'on');
...
plot(ax1, sCommon, qOriginalFC ./ 1000, '--', ...
    'Color', [0.93, 0.49, 0.10], 'LineWidth', 2.8, ...
    'DisplayName', 'Original FC, \gamma_{CO}=0');
plot(ax1, sCommon, qConventional ./ 1000, '-.', ...
    'Color', [0.55, 0.20, 0.70], 'LineWidth', 2.8, ...
    'DisplayName', 'Conventional \gamma, \gamma_{CO}=1');
plot(ax1, sCommon, qOnlyOne ./ 1000, '-', ...
    'Color', [0.86, 0.12, 0.12], 'LineWidth', 3.0, ...
    'DisplayName', 'only\_one yes, \gamma_{CO}=1');
...
title(ax1, 'Orion TPS heat flux with the only\_one model at 80 km');
legend(ax1, 'Location', 'northeastoutside', 'Interpreter', 'tex', ...
    'FontName', 'Times New Roman', 'FontSize', 17, ...
    'LineWidth', 1.2, 'Box', 'off');

%% Figure 2: catalytic heat-flux increments
fig2 = figure('Color', 'w', 'Position', [130, 100, 1050, 760]);
ax2 = axes(fig2);
hold(ax2, 'on');
plot(ax2, sCommon, qReactionOriginal ./ 1000, '--', ...
    'Color', [0.55, 0.20, 0.70], 'LineWidth', 2.8, ...
    'DisplayName', 'Original q_r (\gamma_{CO}=0)');
plot(ax2, sCommon, qReactionConventional ./ 1000, '-.', ...
    'Color', [0.10, 0.45, 0.70], 'LineWidth', 2.8, ...
    'DisplayName', 'Conventional q_r (\gamma_{CO}=1)');
plot(ax2, sCommon, qReactionOnlyOne ./ 1000, '-', ...
    'Color', [0.10, 0.58, 0.22], 'LineWidth', 3.0, ...
    'DisplayName', 'only\_one q_r (\gamma_{CO}=1)');
plot(ax2, sCommon, qOnlyOneDifference ./ 1000, ':', ...
    'Color', [0.93, 0.49, 0.10], 'LineWidth', 2.8, ...
    'DisplayName', 'only\_one - conventional');
formatAxes(ax2);
xlim(ax2, [0, plotArcLengthMax]);
xlabel(ax2, 'Surface arc length from stagnation point, s (m)');
ylabel(ax2, 'Heat-flux increment (kW/m^2)');
title(ax2, 'Catalytic heat-flux increment and only\_one effect at 80 km');
legend(ax2, 'Location', 'best', 'Interpreter', 'tex', ...
    'FontName', 'Times New Roman', 'FontSize', 17, ...
    'LineWidth', 1.2, 'Box', 'off');

%% Figure 3: gamma=0 degradation validation
fig3 = figure('Color', 'w', 'Position', [160, 110, 1050, 820]);
layout = tiledlayout(fig3, 2, 1, 'TileSpacing', 'compact', ...
    'Padding', 'compact');

ax3 = nexttile(layout);
hold(ax3, 'on');
plot(ax3, sCommon, qNC ./ 1000, '-', ...
    'Color', [0.00, 0.32, 0.74], 'LineWidth', 3.0, ...
    'DisplayName', 'NC: no surf\_react');
plot(ax3, sCommon, qGamma0 ./ 1000, '--', ...
    'Color', [0.72, 0.05, 0.55], 'LineWidth', 3.0, ...
    'DisplayName', '\gamma=0');
formatAxes(ax3);
xlim(ax3, [0, plotArcLengthMax]);
ylabel(ax3, 'Heat flux (kW/m^2)');
title(ax3, 'Gamma=0 degradation check at 80 km');
legend(ax3, 'Location', 'best', 'Interpreter', 'tex', ...
    'FontName', 'Times New Roman', 'FontSize', 17, 'Box', 'off');

ax4 = nexttile(layout);
plot(ax4, sCommon, gamma0Difference ./ 1000, '-', ...
    'Color', [0.72, 0.05, 0.05], 'LineWidth', 3.0);
formatAxes(ax4);
xlim(ax4, [0, plotArcLengthMax]);
xlabel(ax4, 'Surface arc length from stagnation point, s (m)');
ylabel(ax4, 'q_{\gamma=0}-q_{NC} (kW/m^2)', 'Interpreter', 'tex');

%% Figure 4: evolution of only_one heat flux with sampling timestep
fcCaseDir = fullfile(onlyOneDir, '80_gCO');
...
if nHistory >= 2
    qHistoryPrevious = interpolateToCommonGrid( ...
        historyResult{end - 1}, result{1}.s);
    historyDifference = result{1}.heatFlux - qHistoryPrevious;
    historyRmsRelativeDifference = sqrt(mean(historyDifference.^2)) / ...
        max(sqrt(mean(result{1}.heatFlux.^2)), eps) * 100;
    historyMaxNormalizedDifference = max(abs(historyDifference)) / ...
        max(max(abs(result{1}.heatFlux)), eps) * 100;
    fprintf(['Latest only_one convergence check: step %d versus step %d\n', ...
        '  RMS relative difference          = %.4g %%\n', ...
        '  Max difference / latest peak |q| = %.4g %%\n'], ...
        historySteps(end), historySteps(end - 1), ...
        historyRmsRelativeDifference, historyMaxNormalizedDifference);
end

historyColors = parula(nHistory);
fig4 = figure('Color', 'w', 'Position', [190, 120, 1050, 760]);
ax5 = axes(fig4);
hold(ax5, 'on');
for iHistory = 1:nHistory
    plot(ax5, historyResult{iHistory}.s, ...
...
formatAxes(ax5);
xlim(ax5, [0, plotArcLengthMax]);
xlabel(ax5, 'Surface arc length from stagnation point, s (m)');
ylabel(ax5, 'Heat flux (kW/m^2)');
title(ax5, sprintf('Evolution of only\\_one heat flux at 80 km (latest: %d steps)', ...
    historySteps(end)));
legend(ax5, 'Location', 'best', 'Interpreter', 'tex', ...
    'FontName', 'Times New Roman', 'FontSize', 17, ...
    'LineWidth', 1.2, 'Box', 'off');

fprintf('\nonly_one heat-flux history steps: %s\n', ...
